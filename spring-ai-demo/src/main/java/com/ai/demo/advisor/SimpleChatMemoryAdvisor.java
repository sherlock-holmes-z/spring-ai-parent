...
@Slf4j
public class SimpleChatMemoryAdvisor implements BaseAdvisor {

    private static final String MESSAGE_KEY = "test_message_key";
    private static final Map<String, List<Message>> chatMemoryMap = new HashMap<>();

    @Override
    public ChatClientRequest before(ChatClientRequest chatClientRequest, AdvisorChain advisorChain) {

        // 通过 chatClientRequest.getConversationId() 获取会话 ID，并将这次的消息记录到内存中
        List<Message> messages = chatMemoryMap.get(MESSAGE_KEY);
        if (messages == null) {
        ...
        chatMemoryMap.put(MESSAGE_KEY, messages);
    ...
    public ChatClientResponse after(ChatClientResponse chatClientResponse, AdvisorChain advisorChain) {
        ...
        // 通过会话 ID，将ai的响应消息记录到内存中
        List<Message> hisMessages = chatMemoryMap.get(MESSAGE_KEY);
